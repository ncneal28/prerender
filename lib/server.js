...
/* Updating the default from false to true in order to handle primary CDCs */
...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		this.finish(req, res);
	//Changing the below 'finally' to 'then' to workaround an issue with prerender 5.16.1
	}).then(() => {
		this.removeRequestFromInFlight(req);
	});
};
